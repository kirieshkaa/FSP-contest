...
from app.domain.interfaces import IPasswordResetRepository
...
class PasswordResetRepository(IPasswordResetRepository):
    ...
    async def get(self, token: str) -> Optional[PasswordResetToken]:
        result = await self._session.execute(
            select(PasswordResetTokenModelDB)
            .where(PasswordResetTokenModelDB.reset_token == UUID(token))
            .where(PasswordResetTokenModelDB.expires_at > datetime.utcnow())
        )
